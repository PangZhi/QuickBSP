...
inline void resetTimer(hrtimer_t* timer)
{
	hrtime_t start = 0;
	hrtime_t end = 0;
}

inline void initTimer(hrtimer_t* timer) 
{
...
inline void startTimer(hrtimer_t* timer)
{
	timer->start = _rdtsc();
}

inline void stopTimer(hrtimer_t* timer)
{
	timer->end = _rdtsc();
}

inline uint64_t getTimerTicks(hrtimer_t* timer)
{
	return timer->end - timer->start;
}

inline uint64_t getTimerNs(hrtimer_t* timer)
{
	if (timer->cpuMHz == 0) {
